...
def main():
    ...
    actual_ratings = [min(5, max(0, item["star"])) for item in dataset]  # Ensure ratings are in [0,5] range
    ...
    # Calculate accuracy
    correct_predictions = sum(1 for actual, predicted in zip(actual_ratings, predicted_ratings) if actual == predicted)
    ...
    for review, actual, predicted in zip(reviews, actual_ratings, predicted_ratings):
        print(f"\nReview: {review[:100]}..." if len(review) > 100 else f"\nReview: {review}")
        print(f"Actual rating:    {rating_to_stars(actual)} ({actual} stars)")
        print(f"Predicted rating: {rating_to_stars(predicted)} ({predicted} stars)")
        print("-" * 80)
